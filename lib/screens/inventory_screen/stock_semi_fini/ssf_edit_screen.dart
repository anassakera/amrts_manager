...
class SsfEditScreen extends StatefulWidget {
  final Map<String, dynamic>? stock;
  final String? lastRefCode;

  const SsfEditScreen({super.key, this.stock, this.lastRefCode});

  @override
  State<SsfEditScreen> createState() => _SsfEditScreenState();
}

class _SsfEditScreenState extends State<SsfEditScreen> {
  ...
  void _initializeControllers() {
    ...
    _itemControllers['weight_per_unit']?.addListener(() {
      setState(() {});
    });
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Text(
                                  widget.stock == null
                                      ? 'Nouveau Stock Semi Fini'
                                      : 'Modifier Stock Semi Fini',
                    ...
                    children: [
                      ...
                      Expanded(
                        child: _buildInfoCard(
                          ...
                          value: _formatNumber(_calculateTotalQuantity()),
                      ...
                      Expanded(
                        child: _buildInfoCard(
                          icon: Icons.scale,
                          label: 'Poids Total',
                          value: '${_formatNumber(_calculateTotalWeight())} KG',
  ...
  Future<void> _saveStock() async {
    ...
    try {
      ...
      final totalAmount = normalizedItems.fold<double>(0.0, (prev, item) {
        final quantity = (item['quantity'] as num?)?.toDouble() ?? 0;
        final unitCost = (item['unit_cost'] as num?)?.toDouble() ?? 0;
        return prev + (quantity * unitCost);
      });
  ...
  void _saveItem() {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final newItem = <String, dynamic>{
        ...
        'unit_cost': unitCost,
        'product_type':
            _itemControllers['product_type']?.text.trim() ?? 'BILLET',
      ...
      child: Form(
        ...
        child: Row(
          children: [
            ...
            _verticalDivider(height: 28),
            _buildCalculatedWeightField(flex: 1),
            _verticalDivider(height: 28),
            _buildEditField(
              'unit_cost',
              'Prix U.',
  ...
  Widget _buildCalculatedWeightField({int flex = 1}) {
    final quantity =
        double.tryParse(_itemControllers['quantity']?.text.trim() ?? '0') ??
        0.0;
    final weightPerUnit =
        double.tryParse(
          _itemControllers['weight_per_unit']?.text.replaceAll(',', '.') ?? '0',
        ) ??
        0.0;
    final calculatedWeight = quantity * weightPerUnit;
