...
class _InvoicesScreenState extends State<InvoicesScreen>
  ...
  // دالة لتحديث البيانات - يمكن استدعاؤها من الخارج
  ...
  Widget _buildInvoicesList({required bool isLocal}) {
    ...
    if (invoices.isEmpty) {
      return RefreshIndicator(
        ...
        child: _buildEmptyState(isLocal, currentLang),
      );
    }
  ...
  Future<void> _addInvoice(Map<String, dynamic> invoice) async {
    ...
    try {
      ...
      final existingInvoice = _invoices.any(
        (inv) =>
      ...
      if (mounted) {
        setState(() {
          final index = _invoices.indexWhere(
            (invoice) =>
                invoice['id'].toString() == updatedInvoice['id'].toString(),
          );
          if (index != -1) {
  ...
  Future<void> _updateInvoiceStatus(String id, String newStatus) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (newStatus == 'Envoyer au stockage' &&
            updatedInvoice['inventory'] != null) {
          ...
          debugPrint('Items processed: $itemsProcessed / $totalItems');
          ...
          if (inventory['errors'] != null &&
              (inventory['errors'] as List).isNotEmpty) {
            message += '\n⚠ بعض المنتجات لم يتم إضافتها بشكل صحيح';
            // طباعة تفاصيل الأخطاء
            debugPrint('Errors: ${inventory['errors']}');
          }
        }
  ...
  Future<void> _loadInvoices() async {
    ...
    // تفريغ القوائم أولاً لإظهار حالة التحميل
    if (mounted) {
      setState(() {
        _invoices = [];
        _filteredInvoices = [];
    ...
    try {
      ...
      if (mounted) {
        ...
        for (final invoice in invoices) {
          ...
          if (id != null &&
              !seenIds.contains(id) &&
              !seenInvoiceNumbers.contains(uniqueKey)) {
            seenIds.add(id);
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final pdfData = invoice['isLocal'] == true
                  ? await PrintInvoiceServiceBuy.generateInvoicesPdf([invoice])
                  : await PrintInvoiceService.generateInvoicesPdf([invoice]);
