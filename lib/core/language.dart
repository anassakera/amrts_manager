class AppTranslations {
  static const Map<String, Map<String, String>> _localizedValues = {
    'ar': {
      'purchases': 'المشتريات',
      'sales': 'المبيعات',
      'inventory': 'المخزون',
      'production': 'الإنتاج',
      'financial_transactions': 'المعاملات المالية',
      'help': 'مساعدة',
      'settings': 'الإعدادات',
      'customers': 'العملاء',
      'notifications': 'الإشعارات',
      'change_language': 'تغيير اللغة',
      'pos': 'نقطة البيع',
      'pos_subtitle': 'بيع المنتجات للعملاء',
      'products': 'المنتجات',
      'products_subtitle': 'إدارة المخزون والمنتجات',
      'sales_invoices': 'فواتير المبيعات',
      'sales_invoices_subtitle': 'إدارة فواتير المبيعات',
      'purchase_invoices': 'فواتير الشراء',
      'purchase_invoices_subtitle': 'إدارة فواتير الشراء',
      'reports': 'التقارير',
      'reports_subtitle': 'تقارير المبيعات والأرباح',
      'customers_subtitle': 'إدارة العملاء',
      'logout': 'تسجيل الخروج',
      'logout_confirmation': 'هل أنت متأكد من تسجيل الخروج؟',
      'version': 'الإصدار 1.0.0',
      'online': 'متصل',
      'system_manager': 'مدير النظام',
      'purchases_subtitle': 'إدارة المشتريات والفواتير',
      'sales_subtitle': 'إدارة المبيعات ونقطة البيع',
      'inventory_subtitle': 'إدارة المخزون والمنتجات',
      'production_subtitle': 'إدارة الإنتاج والتصنيع',
      'financial_transactions_subtitle': 'إدارة المعاملات المالية',
      'smart_invoice': 'فاتورة إلكترونية',
      'number': 'رقم',
      'not_specified': 'غير محدد',
      'items_count': 'عدد العناصر',
      'total_weight': 'الوزن الكلي',
      'total_expenses': 'مجموع المصاريف',
      'goods_total': 'إجمالي البضائع',
      'select_all': 'تحديد الكل',
      'add_new': 'إضافة جديد',
      'delete_selected': 'حذف المحدد',
      'clear_selection': 'إلغاء التحديد',
      'supplier_ref': 'مرجع ',
      'article': 'المادة',
      'quantity': 'الكمية',
      'weight': 'الوزن',
      'unit_price': 'سعر القطعة',
      'total_amount': 'المبلغ الإجمالي',
      'purchase_price': 'سعر الشراء',
      'other_expenses': 'مصاريف أخرى',
      'item_cost': 'تكلفة القطعة',
      'actions': 'الإجراءات',
      'edit': 'تعديل',
      'delete': 'حذف',
      'save': 'حفظ',
      'cancel': 'إلغاء',
      'required': 'مطلوب',
      'invalid_number': 'رقم غير صحيح',
      'integer_only': 'رقم صحيح فقط',
      'expense_details':
          'تفاصيل المصاريف: المرجو إدخال المبالغ بدون رسوم الضريبة على القيمة المضافة',
      'transit': 'النقل',
      'customs': 'حق الجمرك',
      'exchange_cheque': 'شيك الصرف',
      'freight': 'الشحن',
      'other': 'أخرى',
      'exchange_rate': 'سعر الصرف',
      'confirm_delete': 'تأكيد الحذف',
      'delete_selected_items': 'هل تريد حذف العناصر المحددة',
      'delete_this_item': 'هل تريد حذف هذا العنصر؟',
      'enter_value': 'أدخل القيمة',
      'clear_value': 'مسح القيمة',
      // الترجمات الجديدة من ملفات الـ widgets
      'invoice_number': 'رقم الفاتورة',
      'client_name': 'اسم العميل',
      'invoice_date': 'تاريخ الفاتورة',
      'invoice_type': 'نوع الفاتورة',
      'local': 'محلي',
      'external': 'خارجي',
      'local_import': 'شراء محلي',
      'external_import': 'استيراد خارجي',
      'add_new_invoice': 'إضافة فاتورة جديدة',
      'create_invoice': 'إنشاء الفاتورة',
      'add_new_bl': 'إضافة سند تسليم جديد',
      'create_bl': 'إنشاء سند التسليم',
      'add_new_bc': 'إضافة طلبية جديدة',
      'create_bc': 'إنشاء الطلبية',
      'add_new_de': 'إضافة عرض سعر جديد',
      'create_de': 'إنشاء عرض السعر',
      'invoice_date_time': 'تاريخ ووقت الفاتورة',
      'edit_date_time': 'تعديل التاريخ والوقت',
      'please_enter_client_name': 'يرجى إدخال اسم العميل',
      'please_enter_invoice_number': 'يرجى إدخال رقم الفاتورة',
      'close': 'إغلاق',
      'scroll_left': 'تمرير لليسار',
      'scroll_right': 'تمرير لليمين',
      'use_buttons_to_navigate': 'استخدم الأزرار للتنقل بين الأعمدة',
      'items': 'العناصر',
      'invoice_summary': 'ملخص الفاتورة',
      'customs_invoice_number': 'رقم الفاتورة الجمركية',
      'total_invoice': 'إجمالي الفاتورة',
      'total_weight_summary': 'إجمالي الوزن',
      'reference': 'المرجع',
      // ترجمات تسجيل الدخول
      'sign_in': 'تسجيل الدخول',
      'username': 'اسم المستخدم',
      'password': 'كلمة المرور',
      'email': 'البريد الإلكتروني',
      'full_name': 'الاسم الكامل',
      'login': 'دخول',
      'welcome_back': 'مرحباً بعودتك',
      'enter_credentials': 'أدخل بيانات الدخول الخاصة بك',
      'username_required': 'اسم المستخدم مطلوب',
      'password_required': 'كلمة المرور مطلوبة',
      'invalid_credentials': 'اسم المستخدم أو كلمة المرور غير صحيحة',
      'login_success': 'تم تسجيل الدخول بنجاح',
      'login_failed': 'فشل في تسجيل الدخول',
      'remember_me': 'تذكرني',
      'forgot_password': 'نسيت كلمة المرور؟',
      'dont_have_account': 'ليس لديك حساب؟',
      'create_account': 'إنشاء حساب',
      'register': 'تسجيل',
      'already_have_account': 'لديك حساب بالفعل؟',
      'sign_up': 'إنشاء حساب جديد',
      'confirm_password': 'تأكيد كلمة المرور',
      'passwords_not_match': 'كلمات المرور غير متطابقة',
      'password_too_short': 'كلمة المرور يجب أن تكون 6 أحرف على الأقل',
      'invalid_email': 'البريد الإلكتروني غير صحيح',
      'email_already_exists': 'البريد الإلكتروني موجود بالفعل',
      'username_already_exists': 'اسم المستخدم موجود بالفعل',
      'registration_success': 'تم إنشاء الحساب بنجاح',
      'registration_failed': 'فشل في إنشاء الحساب',
      'show_search': 'إظهار البحث',
      'hide_search': 'إخفاء البحث',
      'category': 'الصنف',
      'unit_price_header': 'سعر القطعة',
      'purchase_price_header': 'سعر الشراء',
      'other_expenses_header': 'مصروفات أخرى',
      'total_header': 'الإجمالي',
      'status': 'الحالة',
      'completed': 'مكتملة',
      'pending': 'في الانتظار',
      'draft': 'مسودة',
      'view': 'عرض',
      'print': 'طباعة',
      'update_status': 'تحديث الحالة',
      'update_type': 'تحديث النوع',
      // الترجمات الجديدة من ملف purchases_screen.dart
      'search_invoice_or_client': 'بحث عن فاتورة أو عميل...',
      'advanced_filter': 'فلترة متقدمة',
      'add_invoice': 'إضافة فاتورة',
      'found_results_for_search':
          'تم العثور على {count} نتيجة للبحث: "{query}"',
      'no_invoices': 'لا توجد فواتير',
      'no_local_invoices_created': 'لم يتم إنشاء أي فاتورة استيراد محلي بعد',
      'no_external_invoices_created':
          'لم يتم إنشاء أي فاتورة استيراد خارجي بعد',
      'add_local_invoice': 'إضافة فاتورة محلية',
      'add_external_invoice': 'إضافة فاتورة خارجية',
      'invoice_added_successfully': 'تمت إضافة الفاتورة بنجاح',
      'error_adding_invoice': 'حدث خطأ أثناء إضافة الفاتورة',
      'invoice_updated_successfully': 'تم تحديث الفاتورة بنجاح',
      'error_updating_invoice': 'حدث خطأ أثناء تحديث الفاتورة',
      'invoice_deleted_successfully': 'تم حذف الفاتورة بنجاح',
      'error_deleting_invoice': 'حدث خطأ أثناء حذف الفاتورة',
      'invoice_status_updated_to': 'تم تحديث حالة الفاتورة إلى: {status}',
      'error_updating_invoice_status': 'حدث خطأ أثناء تحديث حالة الفاتورة',
      'invoice_type_updated_to': 'تم تحديث نوع الفاتورة إلى: {type}',
      'error_updating_invoice_type': 'حدث خطأ أثناء تحديث نوع الفاتورة',
      'print_invoice': 'طباعة الفاتورة',
      'do_you_want_to_print_invoice': 'هل تريد طباعة الفاتورة رقم {number}؟',
      'confirm_delete_invoice': 'تأكيد الحذف',
      'are_you_sure_delete_invoice': 'هل أنت متأكد من حذف هذه الفاتورة؟',
      'warning_cannot_undo': 'تحذير: لا يمكن التراجع عن هذا الإجراء',
      'error_loading_invoices': 'حدث خطأ أثناء تحميل الفواتير',
      'error_loading_invoice_details': 'حدث خطأ أثناء تحميل تفاصيل الفاتورة',
      // ترجمات صفحة الملف الشخصي
      'company_profile': 'الملف الشخصي للشركة',
      'company_info': 'معلومات الشركة',
      'legal_name': 'الاسم القانوني',
      'trade_name': 'الاسم التجاري',
      'company_logo': 'شعار الشركة',
      'ice_number': 'رقم التعريف الموحد للشركة',
      'rc_number': 'رقم السجل التجاري',
      'if_number': 'رقم التعريف الجبائي',
      'cnss_number': 'رقم CNSS',
      'address': 'العنوان',
      'city': 'المدينة',
      'country': 'الدولة',
      'phone': 'الهاتف',
      'website': 'الموقع الإلكتروني',
      'edit_profile': 'تعديل الملف الشخصي',
      'save_changes': 'حفظ التغييرات',
      'profile_updated': 'تم تحديث الملف الشخصي بنجاح',
      'error_updating_profile': 'حدث خطأ أثناء تحديث الملف الشخصي',
      'upload_logo': 'رفع الشعار',
      'remove_logo': 'إزالة الشعار',
      'select_image': 'اختيار صورة',
      'camera': 'الكاميرا',
      'gallery': 'المعرض',
      'company_details': 'تفاصيل الشركة',
      'contact_info': 'معلومات الاتصال',
      'legal_info': 'المعلومات القانونية',
      'created_at': 'تاريخ الإنشاء',
      'updated_at': 'آخر تحديث',
      'morocco': 'المغرب',
    },
    'en': {
      'purchases': 'Purchases',
      'sales': 'Sales',
      'inventory': 'Inventory',
      'production': 'Production',
      'financial_transactions': 'Financial Transactions',
      'help': 'Help',
      'settings': 'Settings',
      'customers': 'Customers',
      'notifications': 'Notifications',
      'change_language': 'Change Language',
      'pos': 'Point of Sale',
      'pos_subtitle': 'Sell products to customers',
      'products': 'Products',
      'products_subtitle': 'Manage inventory and products',
      'sales_invoices': 'Sales Invoices',
      'sales_invoices_subtitle': 'Manage sales invoices',
      'purchase_invoices': 'Purchase Invoices',
      'purchase_invoices_subtitle': 'Manage purchase invoices',
      'reports': 'Reports',
      'reports_subtitle': 'Sales and profit reports',
      'customers_subtitle': 'Manage customers',
      'logout': 'Logout',
      'logout_confirmation': 'Are you sure you want to logout?',
      'version': 'Version 1.0.0',
      'online': 'Online',
      'system_manager': 'System Manager',
      'purchases_subtitle': 'Manage purchases and invoices',
      'sales_subtitle': 'Manage sales and point of sale',
      'inventory_subtitle': 'Manage inventory and products',
      'production_subtitle': 'Manage production and manufacturing',
      'financial_transactions_subtitle': 'Manage financial transactions',
      'smart_invoice': 'Smart Invoice',
      'number': 'Number',
      'not_specified': 'Not specified',
      'items_count': 'Items count',
      'total_weight': 'Total weight',
      'total_expenses': 'Total expenses',
      'goods_total': 'Goods total',
      'select_all': 'Select all',
      'add_new': 'Add new',
      'delete_selected': 'Delete selected',
      'clear_selection': 'Clear selection',
      'supplier_ref': ' Ref',
      'article': 'Article',
      'quantity': 'Quantity',
      'weight': 'Weight',
      'unit_price': 'Unit price',
      'total_amount': 'Total amount',
      'purchase_price': 'Purchase price',
      'other_expenses': 'Other expenses',
      'item_cost': 'Item cost',
      'actions': 'Actions',
      'edit': 'Edit',
      'delete': 'Delete',
      'save': 'Save',
      'cancel': 'Cancel',
      'required': 'Required',
      'invalid_number': 'Invalid number',
      'integer_only': 'Integer only',
      'expense_details': 'Expense details (enter amounts without VAT)',
      'transit': 'Transit',
      'customs': 'Customs',
      'exchange_cheque': 'Exchange Cheque',
      'freight': 'Freight',
      'other': 'Other',
      'exchange_rate': 'Exchange rate',
      'confirm_delete': 'Confirm delete',
      'delete_selected_items': 'Delete selected items?',
      'delete_this_item': 'Delete this item?',
      'enter_value': 'Enter value',
      'clear_value': 'Clear value',
      // New translations from widgets files
      'invoice_number': 'Invoice Number',
      'client_name': 'Client Name',
      'invoice_date': 'Invoice Date',
      'invoice_type': 'Invoice Type',
      'local': 'Local',
      'external': 'External',
      'local_import': 'َBuy Local',
      'external_import': 'External Import',
      'add_new_invoice': 'Add New Invoice',
      'create_invoice': 'Create Invoice',
      'add_new_bl': 'Add New Delivery Note',
      'create_bl': 'Create Delivery Note',
      'add_new_bc': 'Add New Purchase Order',
      'create_bc': 'Create Purchase Order',
      'add_new_de': 'Add New Quote',
      'create_de': 'Create Quote',
      'invoice_date_time': 'Invoice Date & Time',
      'edit_date_time': 'Edit Date & Time',
      'please_enter_client_name': 'Please enter client name',
      'please_enter_invoice_number': 'Please enter invoice number',
      'close': 'Close',
      'scroll_left': 'Scroll Left',
      'scroll_right': 'Scroll Right',
      'use_buttons_to_navigate': 'Use buttons to navigate between columns',
      'items': 'Items',
      'invoice_summary': 'Invoice Summary',
      'customs_invoice_number': 'Customs Invoice Number',
      'total_invoice': 'Total Invoice',
      'total_weight_summary': 'Total Weight',
      'reference': 'Reference',
      'show_search': 'Show Search',
      'hide_search': 'Hide Search',
      'category': 'Category',
      'unit_price_header': 'Unit Price',
      'purchase_price_header': 'Purchase Price',
      'other_expenses_header': 'Other Expenses',
      'total_header': 'Total',
      'status': 'Status',
      'completed': 'Completed',
      'pending': 'Pending',
      'draft': 'Draft',
      'view': 'View',
      'print': 'Print',
      'update_status': 'Update Status',
      'update_type': 'Update Type',
      // New translations from purchases_screen.dart
      'search_invoice_or_client': 'Search for invoice or client...',
      'advanced_filter': 'Advanced Filter',
      'add_invoice': 'Add Invoice',
      'found_results_for_search': 'Found {count} results for search: "{query}"',
      'no_invoices': 'No invoices',
      'no_local_invoices_created':
          'No local import invoices have been created yet',
      'no_external_invoices_created':
          'No external import invoices have been created yet',
      'add_local_invoice': 'Add Local Invoice',
      'add_external_invoice': 'Add External Invoice',
      'invoice_added_successfully': 'Invoice added successfully',
      'error_adding_invoice': 'Error occurred while adding invoice',
      'invoice_updated_successfully': 'Invoice updated successfully',
      'error_updating_invoice': 'Error occurred while updating invoice',
      'invoice_deleted_successfully': 'Invoice deleted successfully',
      'error_deleting_invoice': 'Error occurred while deleting invoice',
      'invoice_status_updated_to': 'Invoice status updated to: {status}',
      'error_updating_invoice_status':
          'Error occurred while updating invoice status',
      'invoice_type_updated_to': 'Invoice type updated to: {type}',
      'error_updating_invoice_type':
          'Error occurred while updating invoice type',
      'print_invoice': 'Print Invoice',
      'do_you_want_to_print_invoice':
          'Do you want to print invoice number {number}?',
      'confirm_delete_invoice': 'Confirm Delete',
      'are_you_sure_delete_invoice':
          'Are you sure you want to delete this invoice?',
      'warning_cannot_undo': 'Warning: This action cannot be undone',
      'error_loading_invoices': 'Error occurred while loading invoices',
      'error_loading_invoice_details':
          'Error occurred while loading invoice details',
      // Company profile translations
      'company_profile': 'Company Profile',
      'company_info': 'Company Information',
      'legal_name': 'Legal Name',
      'trade_name': 'Trade Name',
      'company_logo': 'Company Logo',
      'ice_number': 'ICE Number',
      'rc_number': 'RC Number',
      'if_number': 'IF Number',
      'cnss_number': 'CNSS Number',
      'address': 'Address',
      'city': 'City',
      'country': 'Country',
      'phone': 'Phone',
      'website': 'Website',
      'edit_profile': 'Edit Profile',
      'save_changes': 'Save Changes',
      'profile_updated': 'Profile updated successfully',
      'error_updating_profile': 'Error occurred while updating profile',
      'upload_logo': 'Upload Logo',
      'remove_logo': 'Remove Logo',
      'select_image': 'Select Image',
      'camera': 'Camera',
      'gallery': 'Gallery',
      'company_details': 'Company Details',
      'contact_info': 'Contact Information',
      'legal_info': 'Legal Information',
      'created_at': 'Created At',
      'updated_at': 'Updated At',
      'morocco': 'Morocco',
    },
    'fr': {
      'purchases': 'Achats',
      'sales': 'Ventes',
      'inventory': 'Inventaire',
      'production': 'Production',
      'financial_transactions': 'Transactions financières',
      'help': 'Aide',
      'settings': 'Paramètres',
      'customers': 'Clients',
      'notifications': 'Notifications',
      'change_language': 'Changer de langue',
      'pos': 'Point de Vente',
      'pos_subtitle': 'Vendre des produits aux clients',
      'products': 'Produits',
      'products_subtitle': 'Gérer l\'inventaire et les produits',
      'sales_invoices': 'Factures de Vente',
      'sales_invoices_subtitle': 'Gérer les factures de vente',
      'purchase_invoices': 'Factures d\'Achat',
      'purchase_invoices_subtitle': 'Gérer les factures d\'achat',
      'reports': 'Rapports',
      'reports_subtitle': 'Rapports de ventes et de profits',
      'customers_subtitle': 'Gérer les clients',
      'logout': 'Déconnexion',
      'logout_confirmation': 'Êtes-vous sûr de vouloir vous déconnecter ?',
      'version': 'Version 1.0.0',
      'online': 'En ligne',
      'system_manager': 'Gestionnaire de Système',
      'purchases_subtitle': 'Gérer les achats et factures',
      'sales_subtitle': 'Gérer les ventes et point de vente',
      'inventory_subtitle': 'Gérer l\'inventaire et produits',
      'production_subtitle': 'Gérer la production et fabrication',
      'financial_transactions_subtitle': 'Gérer les transactions financières',
      'smart_invoice': 'Facture intelligente',
      'number': 'Numéro',
      'not_specified': 'Non spécifié',
      'items_count': 'Nombre d\'articles',
      'total_weight': 'Poids total',
      'total_expenses': 'Total des dépenses',
      'goods_total': 'Total marchandises',
      'select_all': 'Tout sélectionner',
      'add_new': 'Ajouter',
      'delete_selected': 'Supprimer la sélection',
      'clear_selection': 'Effacer la sélection',
      'supplier_ref': 'Réf',
      'article': 'Article',
      'quantity': 'Quantité',
      'weight': 'Poids',
      'unit_price': 'Prix unitaire',
      'total_amount': 'Montant total',
      'purchase_price': 'Prix d\'achat',
      'other_expenses': 'Autres frais',
      'item_cost': 'Coût de l\'article',
      'actions': 'Actions',
      'edit': 'Modifier',
      'delete': 'Supprimer',
      'save': 'Enregistrer',
      'cancel': 'Annuler',
      'required': 'Obligatoire',
      'invalid_number': 'Nombre invalide',
      'integer_only': 'Entier seulement',
      'expense_details': 'Détails des dépenses (saisir les montants hors TVA)',
      'transit': 'Transport',
      'customs': 'Droit de douane',
      'exchange_cheque': 'Chèque de change',
      'freight': 'Fret',
      'other': 'Autre',
      'exchange_rate': 'Taux de change',
      'confirm_delete': 'Confirmer la suppression',
      'delete_selected_items': 'Supprimer les éléments sélectionnés ?',
      'delete_this_item': 'Supprimer cet élément ?',
      'enter_value': 'Entrer la valeur',
      'clear_value': 'Effacer la valeur',
      // Nouvelles traductions des fichiers widgets
      'invoice_number': 'Numéro de facture',
      'client_name': 'Nom du fournisseur',
      'invoice_date': 'Date de facture',
      'invoice_type': 'Type de facture',
      'local': 'Local',
      'external': 'Externe',
      'local_import': 'Achat Local',
      'external_import': 'Import externe',
      'add_new_invoice': 'Ajouter une nouvelle facture',
      'create_invoice': 'Créer la facture',
      'add_new_bl': 'Ajouter un nouveau Bon de Livraison',
      'create_bl': 'Créer le BL',
      'add_new_bc': 'Ajouter un nouveau Bon de Commande',
      'create_bc': 'Créer le BC',
      'add_new_de': 'Ajouter un nouveau Devis',
      'create_de': 'Créer le Devis',
      'invoice_date_time': 'Date et heure de facture',
      'edit_date_time': 'Modifier la date et l\'heure',
      'please_enter_client_name': 'Veuillez saisir le nom du client',
      'please_enter_invoice_number': 'Veuillez saisir le numéro de facture',
      'close': 'Fermer',
      'scroll_left': 'Défiler à gauche',
      'scroll_right': 'Défiler à droite',
      'use_buttons_to_navigate':
          'Utilisez les boutons pour naviguer entre les colonnes',
      'items': 'Articles',
      'invoice_summary': 'Résumé de la facture',
      'customs_invoice_number': 'Numéro de facture douanière',
      'total_invoice': 'Total de la facture',
      'total_weight_summary': 'Poids total',
      'reference': 'Référence',
      'show_search': 'Afficher la recherche',
      'hide_search': 'Masquer la recherche',
      'category': 'Catégorie',
      'unit_price_header': 'Prix unitaire',
      'purchase_price_header': 'Prix d\'achat',
      'other_expenses_header': 'Autres frais',
      'total_header': 'Total',
      'status': 'Statut',
      'completed': 'Terminée',
      'pending': 'En attente',
      'draft': 'Brouillon',
      'view': 'Voir',
      'print': 'Imprimer',
      'update_status': 'Mettre à jour le statut',
      'update_type': 'Mettre à jour le type',
      // Nouvelles traductions de purchases_screen.dart
      'search_invoice_or_client': 'Rechercher une facture ou un client...',
      'advanced_filter': 'Filtre avancé',
      'add_invoice': 'Ajouter une facture',
      'found_results_for_search':
          'Trouvé {count} résultats pour la recherche : "{query}"',
      'no_invoices': 'Aucune facture',
      'no_local_invoices_created':
          'Aucune facture d\'import local n\'a été créée',
      'no_external_invoices_created':
          'Aucune facture d\'import externe n\'a été créée',
      'add_local_invoice': 'Ajouter une facture locale',
      'add_external_invoice': 'Ajouter une facture externe',
      'invoice_added_successfully': 'Facture ajoutée avec succès',
      'error_adding_invoice': 'Erreur lors de l\'ajout de la facture',
      'invoice_updated_successfully': 'Facture mise à jour avec succès',
      'error_updating_invoice': 'Erreur lors de la mise à jour de la facture',
      'invoice_deleted_successfully': 'Facture supprimée avec succès',
      'error_deleting_invoice': 'Erreur lors de la suppression de la facture',
      'invoice_status_updated_to':
          'Statut de la facture mis à jour vers : {status}',
      'error_updating_invoice_status':
          'Erreur lors de la mise à jour du statut de la facture',
      'invoice_type_updated_to': 'Type de facture mis à jour vers : {type}',
      'error_updating_invoice_type':
          'Erreur lors de la mise à jour du type de facture',
      'print_invoice': 'Imprimer la facture',
      'do_you_want_to_print_invoice':
          'Voulez-vous imprimer la facture numéro {number} ?',
      'confirm_delete_invoice': 'Confirmer la suppression',
      'are_you_sure_delete_invoice':
          'Êtes-vous sûr de vouloir supprimer cette facture ?',
      'warning_cannot_undo':
          'Attention : Cette action ne peut pas être annulée',
      'error_loading_invoices': 'Erreur lors du chargement des factures',
      'error_loading_invoice_details':
          'Erreur lors du chargement des détails de la facture',
      // Traductions du profil de l'entreprise
      'company_profile': 'Profil de l\'Entreprise',
      'company_info': 'Informations de l\'Entreprise',
      'legal_name': 'Nom Légal',
      'trade_name': 'Nom Commercial',
      'company_logo': 'Logo de l\'Entreprise',
      'ice_number': 'Numéro ICE',
      'rc_number': 'Numéro RC',
      'if_number': 'Numéro IF',
      'cnss_number': 'Numéro CNSS',
      'address': 'Adresse',
      'city': 'Ville',
      'country': 'Pays',
      'phone': 'Téléphone',
      'website': 'Site Web',
      'edit_profile': 'Modifier le Profil',
      'save_changes': 'Enregistrer les Modifications',
      'profile_updated': 'Profil mis à jour avec succès',
      'error_updating_profile': 'Erreur lors de la mise à jour du profil',
      'upload_logo': 'Télécharger le Logo',
      'remove_logo': 'Supprimer le Logo',
      'select_image': 'Sélectionner une Image',
      'camera': 'Caméra',
      'gallery': 'Galerie',
      'company_details': 'Détails de l\'Entreprise',
      'contact_info': 'Informations de Contact',
      'legal_info': 'Informations Légales',
      'created_at': 'Créé le',
      'updated_at': 'Mis à jour le',
      'morocco': 'Maroc',
    },
  };

  static String get(String key, String lang) {
    return _localizedValues[lang]?[key] ?? _localizedValues['ar']![key] ?? key;
  }
}
